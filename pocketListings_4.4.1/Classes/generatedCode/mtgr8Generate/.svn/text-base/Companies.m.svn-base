...
- (NSMutableArray*) company_var{
	   NSString* prefix = [XMLUtil getPrefix:[self getURI] dictionary:nsMappings];	
		if( company_var == nil){
		id resXML= [XMLUtil getXMLValue:[NSString stringWithFormat:@"%@:%@",prefix,company_var_XMLNAME] dictionary:nsMappings inNode:xml];
		if( resXML != nil){
			self.company_var = [[NSMutableArray alloc] init];
			if( [resXML isKindOfClass:[CXMLElement class]]){
									[self.company_var addObject:[[[Company alloc] initWithXML:resXML] autorelease]];		
							}else if( [resXML isKindOfClass:[NSArray class]]){
				for(int i=0;i<[resXML count];i++){
									[self.company_var addObject:[[[Company alloc] initWithXML:[resXML objectAtIndex:i]] autorelease]];		
								}
			}
		}
	}
...
- (NSString *)serializeToXMLString:(NSString *)elemName nameSpaces:(NSMutableDictionary *)nameSpaces{
	...
	- (NSString *)serializedElementString:(NSMutableDictionary *)nameSpaces
	{
		NSMutableString *serializedForm = [NSMutableString string];
		    			...
		    			for(int i=0;i<[self.company_var count];i++){
			    					[serializedForm appendFormat:@"%@", [[self.company_var objectAtIndex:i] serializeToXMLString:self.company_var_XMLNAME nameSpaces:nameSpaces]];		
							}
				 	} 
