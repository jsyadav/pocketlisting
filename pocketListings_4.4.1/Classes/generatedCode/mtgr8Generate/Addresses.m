...
@synthesize address_var;
@synthesize address_var_XMLNAME;
- (Address*) address_var{
	   NSString* prefix = [XMLUtil getPrefix:[self getURI] dictionary:nsMappings];	
		if( address_var == nil){
		id objXML = [XMLUtil getXMLValue:[NSString stringWithFormat:@"%@:%@",prefix,address_var_XMLNAME] dictionary:nsMappings inNode:xml];
		if( objXML != nil){
			self.address_var=[[Address   alloc] initWithXML:objXML];
		}
	}
...
- (id)initWithXML:(CXMLNode*)axml { 
	if(self = [super init]){
		...
		id var=nil;
		id res=nil;
	    self.address_var_XMLNAME=@"Address";
	self.address_var=nil;
			}
	return self;
}

- (void)dealloc {
	[address_var_XMLNAME release];
	[address_var release];
	[nsMappings release];
	[xml release];
...
- (NSString *)serializeToXMLString:(NSString *)elemName nameSpaces:(NSMutableDictionary *)nameSpaces{
	...
	- (NSString *)serializedElementString:(NSMutableDictionary *)nameSpaces
	{
		NSMutableString *serializedForm = [NSMutableString string];
						...
						if(self.address_var!=nil){
		    			[serializedForm appendFormat:@"%@", [self.address_var serializeToXMLString:self.address_var_XMLNAME nameSpaces:nameSpaces]];		
				 	} 
