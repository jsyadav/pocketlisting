...
static MURLCache*          gSharedCache = nil;
...
- (id)initWithName:(NSString*)name {
    ...
    if (self) {
        _name             = [name copy];
        _cachePath        = [[MURLCache cachePathWithName:name] retain];
        _invalidationAge  = (60*60*24);
...
///////////////////////////////////////////////////////////////////////////////////////////////////
+ (MURLCache*)cacheWithName:(NSString*)name {
    if (nil == gNamedCaches) {
        gNamedCaches = [[NSMutableDictionary alloc] init];
    }
    MURLCache* cache = [gNamedCaches objectForKey:name];
    if (nil == cache) {
        cache = [[[MURLCache alloc] initWithName:name] autorelease];
...
///////////////////////////////////////////////////////////////////////////////////////////////////
+ (MURLCache*)sharedCache {
    if (nil == gSharedCache) {
        gSharedCache = [[MURLCache alloc] init];
    }
    return gSharedCache;
}
...
///////////////////////////////////////////////////////////////////////////////////////////////////
+ (void)setSharedCache:(MURLCache*)cache {
...
- (BOOL)hasImageForURL:(NSString*)URL fromDisk:(BOOL)fromDisk {
    ...
    if (!hasImage && fromDisk) {
        if (TTIsBundleURL(URL)) {
            ...
            if (!hasImage) {
                hasImage = [self imageExistsFromBundle:[MURLCache doubleImageURLPath:URL]];
            }
            
        } else if (TTIsDocumentsURL(URL)) {
            hasImage = [self imageExistsFromDocuments:URL];
            if (!hasImage) {
                hasImage = [self imageExistsFromDocuments:[MURLCache doubleImageURLPath:URL]];
            }
...
- (void)removeAll:(BOOL)fromDisk {
    ...
    if (fromDisk) {
        ...
        [MURLCache createPathIfNecessary:_cachePath];
    }
}
