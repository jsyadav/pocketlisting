...
- (void)connectToURL:(NSURL*)URL {
    ...
    MURLRequest* request = _requests.count == 1 ? [_requests objectAtIndex:0] : nil;
...
- (void)dispatchLoadedBytes:(NSInteger)bytesLoaded expected:(NSInteger)bytesExpected {
    for (MURLRequest* request in [[_requests copy] autorelease]) {
...
- (NSError*)processResponse:(NSHTTPURLResponse*)response data:(id)data {
    for (MURLRequest* request in _requests) {
        NSError* error = nil;
...
- (void)connection:(NSURLConnection*)connection didReceiveResponse:(NSHTTPURLResponse*)response {
    ...
    // [[MURLRequestQueue mainQueue] setMaxContentLength:0]
